...
rev 1.0
rev 2.0: 
rev 3.0: change PretrainedModel class to work with pretrained model on unnorm dataset
...
def load_predefined_set(filename,flag):
    ...
    if flag:
        ...
        # out: [ocla, oID]
        np_df = df.drop(['tcla', 'tID','idx'], axis=1)
        np_df = np_df.drop_duplicates(subset=['ocla'],keep='first')
...
def get_evalset(model,dataset,net,input_set,seed,flag):
    if dataset == 'imagenet':
        if net == 'vit':
            subset_path = 'vit_dataset_200_final_seed0.csv'
        elif net == 'resnet50':
            subset_path = 'ImageNet-220 oclass-5 tclass-final.csv'
